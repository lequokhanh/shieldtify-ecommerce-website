...
const Products = () => {
	...
	useEffect(() => {
		async function fetchData() {
			...
			await getAllProductByCategory({
				searchValue: "getTotalProducts",
			}).then((res) => {
		...
		<Flex flexDir="column" padding="24px 34px" w="full" gap="30px">
			...
			<HStack gap="24px">
				<InfoCard
					name="Products"
					icon={apps}
					info={totalProducts || <Spinner />}
					bgColor="shieldtify.grey.400"
					iconColor="#444444"
				/>
				<InfoCard
					name="Categories"
					icon={label}
					info={
						(categories.length && categories.length) || <Spinner />
					}
					bgColor="#FDF2F2"
					iconColor="#FF6262"
				/>
				<InfoCard
					name="Brands"
					icon={crown}
					info={(allBrands && allBrands.length) || <Spinner />}
					bgColor="#FEF9EE"
					iconColor="#D29211"
				/>
			</HStack>
					...
					<Flex justifyContent="space-between">
						<Flex alignItems="flex-end">
							<Box
								...
								style={{
									borderBottom: isOnProducts
										? "2px solid #444444"
										: "",
									borderRadius: isOnProducts
										? "2px 2px 0px 0px"
										: "",
								}}
							...
							>
								<Text
									fontSize="0.875rem"
									fontWeight={isOnProducts ? "700" : "400"}
									color="#565D6D"
								>
									Products
								</Text>
									...
									<>
										...
										<PopoverContent>
											<BrandsPopoverContent
												unsubmittedCheckedBrands={
													unsubmittedCheckedBrands
												}
												setUnsubmittedCheckedBrands={
													setUnsubmittedCheckedBrands
												}
											/>
										</PopoverContent>
			...
			<CategoryCreateModal
				...
				checkedCategory={
					checkedCategories.length === 1 && checkedCategories[0]
				}
				editType={editType}
			/>
		</Flex>
