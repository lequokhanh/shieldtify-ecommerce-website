...
const EditUsers = () => {
	...
	const [isOnAddresses, setIsOnAddresses] = useState(true);
	...
	const handleDeleteAddress = async () => {
		await deleteAddress({ addID: selectedAddress.uid, userID: id })
			.then(() => {
				setAddresses(
					addresses.filter((add) => add.uid !== selectedAddress.uid)
				);
				toast({
					title: "Success",
	...
	const handleAddressesClick = () => {
		setIsOnAddresses(true);
	};
	const handleChatHistoriesClick = () => {
		setIsOnAddresses(false);
	};
	return (
		profile && (
			<Flex
				...
				<Flex flexDir={"column"} gap="20px" w="full">
					<Flex alignItems="flex-end">
						<Box
							...
							style={{
								borderBottom: isOnAddresses
									? "2px solid #444444"
									: "",
								borderRadius: isOnAddresses
									? "2px 2px 0px 0px"
									: "",
							}}
						...
						>
							<Text
								fontSize="0.875rem"
								fontWeight={isOnAddresses ? "700" : "400"}
								color="#565D6D"
							>
								Addresses
							</Text>
						...
						<Box
							paddingX="22px"
							onClick={handleChatHistoriesClick}
							_hover={{ cursor: "pointer" }}
							paddingBottom="5px"
							style={{
								borderBottom: !isOnAddresses
