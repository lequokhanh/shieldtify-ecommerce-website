...
export function getAllProductByCategory({category='', page = 1, brands, searchValue}) {
    let url
    ...
    if (brands !== null && brands !== undefined && brands !== '' && brands.length > 0) {
...
export function updateProduct({productID,categoryid,stock_qty,name,brandid,price,specification,description}) {
...
export function updateProductForStaff({productID,categoryid,stock_qty,name,brandid,price,specification,description}) {
...
export function setPrimaryImage({productID,imgid}) {
    return axiosCookie.put(`/product/image/default/${productID}?imgid=${imgid}`);
}

export function createNewProduct({categoryid,stock_qty,name,brandid,price,specification,description}) {
...
export function updateAddress({add, userID}) {
    ...
    return axiosCookie.put(`/user/address/admin/${userID}?addressId=${add.uid}`, {
        address: add.address,
        city: add.city,
...
export function deleteAddress({addID, userID}) {
    return axiosCookie.delete(`/user/address/admin/${userID}?addressId=${addID}`)
}
...
export function getAllClients({page=1,keyword=''}){
    return axiosCookie.get(`/user/client/admin?page=${page}&keyword=${keyword}`);   
}
